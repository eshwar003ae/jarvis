...
# --- ALIASES FOR BRAIN.PY COMPATIBILITY ---

# The brain looks for 'open', so we point it to your 'open_file' function
open = open_file

# The brain looks for 'folder_file', so we point it to your 'list_folder_items' function
folder_file = list_folder_items

# The brain looks for 'close', so we create a simple close function
async def close(app_name: str):
    """Closes an application by name."""
    import os
    os.system(f"taskkill /f /im {app_name}.exe")
    return {"ok": True, "action": f"Closed {app_name}"}
